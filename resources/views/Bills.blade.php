...
<body class="bg-gray-950 text-gray-100 min-h-screen flex flex-col">
        ...
        <div class="relative">
          ...
          <div id="notificationDropdown"
            ...
            <div class="p-3 text-center border-t border-gray-700">
              <button class="text-green-400 hover:underline" onclick="clearNotifications()">Clear All</button>
            </div>
          </div>
        </div>
      </nav>
  ...
  <main class="flex-1 max-w-7xl mx-auto p-6 w-full">
    ...
    <div id="billsGrid" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6"></div>
  </main>
  ...
  <script>
    ...
    notificationBtn.addEventListener("click", () => {
      ...
      notificationBadge.classList.add("hidden"); // mark seen
    });
    ...
    async function loadBills() {
      let res = await fetch('/api/bills', { headers: { 'Authorization': 'Bearer ' + token } });
      let data = await res.json();
      if (res.ok) {
        billsGrid.innerHTML = "";
        data.forEach(bill => billsGrid.innerHTML += renderBillCard(bill));
      }
    }
    ...
    billForm.addEventListener("submit", async (e) => {
      ...
      if (res.ok) {
        closeModal();
        loadBills();
        addNotification(editingBillId ? `Bill updated: ${billData.name}` : `New bill added: ${billData.name}`);
      }
    });

    // Edit Bill
    async function editBill(id) {
      let res = await fetch(`/api/bills/${id}`, { headers: { 'Authorization': 'Bearer ' + token } });
      if (res.ok) {
    ...
    async function deleteBill(id) {
      if (confirm("Are you sure you want to delete this bill?")) {
        let res = await fetch(`/api/bills/${id}`, { method: "DELETE", headers: { 'Authorization': 'Bearer ' + token } });
        if (res.ok) {
          loadBills();
          addNotification(`Bill deleted (ID: ${id})`);
        }
      }
    }
